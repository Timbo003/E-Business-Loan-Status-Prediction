...
# Define a custom summary function to include a cost matrix
custom_summary <- function(data, lev = NULL, model = NULL) {
  ...
  cost_matrix <- matrix(c(0, 20, 1, 0), nrow = 2)
  colnames(cost_matrix) <- rownames(cost_matrix) <- c("no", "yes")
  
  # Calculate the total cost
  total_cost <- fp * cost_matrix["no", "yes"] + fn * cost_matrix["yes", "no"]
  ...
  out <- c(confusion$overall, confusion$byClass, TotalCost = total_cost, F1 = f1)
  names(out) <- c(names(confusion$overall), names(confusion$byClass), "TotalCost", "F1")
  
  return(out)
}

# Define control for Random Forest with cross-validation using the custom summary function
...
# Define an expanded grid for Random Forest with additional hyperparameters
rf_grid <- expand.grid(
  mtry = c(2, 4, 6, 8),          # Number of variables to try at each split
  ntree = c(100, 200, 500),      # Number of trees in the forest
  maxnodes = c(10, 20, 30)       # Maximum number of terminal nodes trees can have
)

# Custom train function to handle additional parameters
train_rf_model <- function(data, tuneGrid) {
  best_model <- NULL
  best_cost <- Inf
  for (i in 1:nrow(tuneGrid)) {
    ...
    model <- train(
      ...
      metric = "TotalCost" # Optimize based on the total cost
    )
    if (model$results$TotalCost < best_cost) {
      best_cost <- model$results$TotalCost
      best_model <- model
    }
  }
  return(best_model)
...
# Train the Random Forest model with expanded grid
rf_model <- train_rf_model(X_train, rf_grid)

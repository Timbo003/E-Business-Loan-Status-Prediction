...
avg_loan_requested <- rawData %>%
  group_by(Toal_Income) %>%
  summarize(LoanAmount = mean(LoanAmount, na.rm = TRUE))

# Plotting average loan amount requested with a trend line
ggplot(avg_loan_requested, aes(x = Toal_Income, y = LoanAmount)) +
  geom_line() +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "blue") +
  ggtitle("Average Loan Amount Requested by Total_Income") +
  xlab("Applicant TotalIncome") +
  ylab("Average Loan Amount") +
  theme_minimal()
...
# Plot 21: Average Loan Amount requested by Loan_Amount_Term
avg_loan_amount <- rawData %>%
...
# Plot 22: Average Loan Amount granted by Loan_Amount_Term
avg_loan_amount <- rawData %>%
  filter(Loan_Status == "TRUE") %>%
  group_by(Loan_Amount_Term) %>%
  summarize(LoanAmount = mean(LoanAmount))
...
# Plot 21: Average Loan Amount requested by Loan_Status
avg_loan_amount <- rawData %>%
...
# predTest = predict(mod, newdata=churn.test, type="response")
# 
# #ROC AND AUC
# rocr.log.pred <- prediction(predTest, churn.test$Churn)
# logPerformance <- performance(rocr.log.pred, "tpr", "fpr")
# plot(logPerformance, colorize = TRUE)
# abline(0, 1)
# logAuc<-performance(rocr.log.pred,"auc")
# logAuc@y.values
